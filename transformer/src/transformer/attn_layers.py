...
def batch_multi_head_attention(
    ...
    *,
    ...
    Args:
        queries: Tensor of shape (bsz, n_head, l_x, d_attn). A sequence of queries.
        keys: Tensor of shape (bsz, n_head, l_z, d_attn). A sequence of keys.
        values: Tensor of shape (bsz, n_head, l_z, d_out). A sequence of values.
        attn_mask: Optional tensor broadcastable to shape (bsz, n_head, l_x, l_z).
            Boolean or additive attention mask. If boolean, attention is allowed where True.
            If not boolean, attn_mask is added to scores before softmax.
        scale: Optional float.
            Scaling factor for attention. Default value is 1/sqrt(d_attn).
    ...
    Returns:
        dict:
            * scores: Tensor of shape (bsz, n_head, l_x, l_z). Attention scores (pre softmax)
            * attn: Tensor of shape (bsz, n_head, l_x, l_z). Attention weights (post softmax)
            * output: Tensor of shape (bsz, n_head, l_x, d_out). Attention-weighted output tensor
    ...
    assert (
        keys.dim() == 4
        and keys.shape[0] == bsz
        and keys.shape[1] == n_head
        and keys.shape[3] == d_attn
    )
    l_z = keys.shape[2]

    assert (
        values.dim() == 4
        and values.shape[0] == bsz
        and values.shape[1] == n_head
        and values.shape[2] == l_z
    )
    ...
    scores = torch.einsum("bnqd, bnkd -> bnqk", queries, keys) * scale
    assert scores.shape == (bsz, n_head, l_x, l_z)
    neg_inf = float("-inf")

    if attn_mask is not None:
        assert attn_mask.shape == (bsz, n_head, l_x, l_z)
